...
export async function GET(request: NextRequest) {
  ...
  return bffHandler(() => adminGet<Paginated<TestimonialAdmin>>(ENDPOINTS.ADMIN.TESTIMONIALS, params));
}

export async function POST(request: NextRequest) {
  const body = await parseJsonBody<Record<string, unknown>>(request);
  if (!body) return bffHandler(async () => Promise.reject(new Error("Empty body")));
  return bffHandler(() => adminPost<TestimonialAdmin>(ENDPOINTS.ADMIN.TESTIMONIALS, body));
}
