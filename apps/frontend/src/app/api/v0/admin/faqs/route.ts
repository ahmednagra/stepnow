...
import type { FaqCreateInput } from "@/services/faqs/faqs.admin.client";
...
export async function GET(request: NextRequest) {
  ...
  try {
    return NextResponse.json(await listAdminFaqsServer(params, token));
  } catch (err) {
...
export async function POST(request: NextRequest) {
  const token = extractBearerToken(request);
  if (!token) return errorResponse("UNAUTHORIZED", "Authentication token is required", 401);
  const body = await parseJsonBody<FaqCreateInput>(request);
  if (!body) return errorResponse("BAD_REQUEST", "Empty body", 400);
  try {
    return NextResponse.json(await createAdminFaqServer(body, token), { status: 201 });
  } catch (err) {
    return apiErrorResponse(err);
  }
}
