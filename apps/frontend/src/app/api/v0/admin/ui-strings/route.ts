...
export async function GET(request: NextRequest) {
  ...
  return bffHandler(() => adminGet<Paginated<UiStringAdmin>>(ENDPOINTS.ADMIN.UI_STRINGS, params));
}

export async function POST(request: NextRequest) {
  const body = await parseJsonBody<Record<string, unknown>>(request);
  if (!body) return bffHandler(async () => Promise.reject(new Error("Empty body")));
  return bffHandler(() => adminPost<UiStringAdmin>(ENDPOINTS.ADMIN.UI_STRINGS, body));
}
