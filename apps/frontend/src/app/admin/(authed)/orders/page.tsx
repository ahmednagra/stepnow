...
export default function OrdersPage() {
  ...
  return (
    <>
      <AdminPageHeader eyebrow="Operations" title="Orders" description="Confirmed jobs converted from bookings." />
      <div className="space-y-4 p-6">
        <FilterToolbar
          ...
          exports={{
            onCsv: () => orders && exportCsv(orders.map((o) => ({
              order_number: o.order_number, customer: o.customer_name, email: o.customer_email,
              pickup: o.pickup_address, destination: o.destination_address,
              scheduled: o.scheduled_datetime ?? "", status: o.status,
              net: o.net_amount, vat: o.vat_amount, gross: o.gross_amount, created_at: o.created_at,
        ...
        <div id="orders-printable">
          <AdminCard flush title={`${total} ${total === 1 ? "order" : "orders"}`}>
            <AdminTable columns={["Order-No.", "Customer", "Route", "Scheduled", "Status", "Gross"]}>
              {loading ? (
                <AdminTableEmpty loading />
              ) : !orders || orders.length === 0 ? (
                <AdminTableEmpty message="No orders yet. Convert a booking to create one." />
              ) : (
                orders.map((o) => (
                  <AdminTableRow key={o.id}>
                    <AdminTableCell>
                      <Link href={`/admin/orders/${o.id}`} className="font-mono text-[11.5px] font-medium text-slate-900 hover:underline">
                        {o.order_number}
                      </Link>
                    </AdminTableCell>
                    <AdminTableCell>
                    ...
                    <AdminTableCell>
                      {o.scheduled_datetime ? (
                        <time className="text-[11.5px] tabular-nums text-slate-700" dateTime={o.scheduled_datetime}>
                          {new Date(o.scheduled_datetime).toLocaleString("en-GB", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}
                        </time>
                      ) : <span className="text-slate-400">—</span>}
                    </AdminTableCell>
                    <AdminTableCell><OrderStatusBadge status={o.status} /></AdminTableCell>
                    <AdminTableCell>
                      <span className="font-serif text-[14px] tabular-nums text-slate-900">{formatPriceEur(o.gross_amount)}</span>
                    </AdminTableCell>
                  </AdminTableRow>
                ))
              )}
