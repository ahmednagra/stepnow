...
// Vehicles list. Search input now debounced 300ms via inline setTimeout cleanup (no new hook file).
...
<Link href="/admin/vehicles/new" className="flex h-9 items-center gap-1.5 bg-slate-900 px-3.5 text-[12.5px] font-medium text-white transition-colors hover:bg-slate-800">
...
<Link href={`/admin/vehicles/${v.id}`} className="font-medium text-slate-900 hover:underline">{v.name_de}</Link>
<p className="text-[11px] text-slate-500 truncate">{v.name_en}</p>
</div>
</div>
</AdminTableCell>
<AdminTableCell>
<span className="inline-block bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wider text-slate-700">{v.category}</span>
</AdminTableCell>
<AdminTableCell>
<span className="text-[12px] tabular-nums text-slate-700">{v.capacity_passengers}p · {v.capacity_luggage}l</span>
</AdminTableCell>
<AdminTableCell>
{v.is_deleted ? (
