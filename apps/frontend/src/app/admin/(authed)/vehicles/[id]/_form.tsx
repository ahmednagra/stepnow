// src/app/admin/(authed)/vehicles/[id]/_form.tsx
"use client";
...
function toPayload(values: AdminVehicleInput): VehicleCreateInput {
  ...
  return {
    ...
    features_de: (values.features_de ?? "")
      .split("\n")
      .map((s) => s.trim())
      .filter(Boolean),
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
      ...
      <AdminCard title="Identity">
        <div className="flex flex-col gap-4">
          <BilingualField
            ...
            de={<input className={adminInputClass} {...register("name_de")} disabled={isDeleted} />}
            en={<input className={adminInputClass} {...register("name_en")} disabled={isDeleted} />}
          />
          <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
            <AdminFormField label="Category" required error={errors.category?.message}>
              <input
                placeholder="sedan, van, minibus…"
            ...
            <AdminFormField
              ...
              required
              error={errors.capacity_passengers?.message}
            >
              <input
                type="number"
                min="1"
      ...
      <AdminCard title="Features" description="One feature per line.">
        <BilingualField
          label="Features"
          errorDe={errors.features_de?.message}
          errorEn={errors.features_en?.message}
          de={
            <textarea
              rows={6}
