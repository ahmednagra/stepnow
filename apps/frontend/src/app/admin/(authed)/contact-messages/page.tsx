...
filters={
<select value={handled} onChange={(e) => setHandled(e.target.value as HandledFilter)} className="h-8 border border-slate-300 bg-white px-2 text-[12px] text-slate-700" aria-label="Filter by status">
...
<button type="button" onClick={() => markSelected(true)} className="flex h-8 items-center gap-1.5 border border-emerald-200 bg-white px-2.5 text-[11.5px] font-medium text-emerald-700 hover:bg-emerald-50">
<CheckCircle2 className="h-3 w-3" strokeWidth={1.5} aria-hidden="true" />
Mark handled
</button>
<button type="button" onClick={() => markSelected(false)} className="flex h-8 items-center gap-1.5 border border-slate-300 bg-white px-2.5 text-[11.5px] font-medium text-slate-700 hover:bg-slate-50">
<Circle className="h-3 w-3" strokeWidth={1.5} aria-hidden="true" />
Mark unhandled
</button>
<button type="button" onClick={() => setSelected(new Set())} className="h-8 px-2.5 text-[11.5px] font-medium text-slate-500 hover:text-slate-900">
Clear
</button>
</div>
</div>
...
<AdminCard flush title={`${pagination?.total ?? 0} ${pagination?.total === 1 ? "message" : "messages"}`}>
...
<input type="checkbox" aria-label="Select" checked={selected.has(m.id)} onChange={() => toggle(m.id)} className="h-3.5 w-3.5" />
</AdminTableCell>
<AdminTableCell className="w-8 align-top">
{m.is_handled ? (
...
<Link href={`/admin/contact-messages/${m.id}`} className="font-medium text-slate-900 hover:underline">{m.name}</Link>
...
<AdminTableCell className="align-top">
<span className="inline-block bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wider text-slate-700">{m.subject_category}</span>
</AdminTableCell>
<AdminTableCell className="align-top">
...
<Link href={`/admin/contact-messages/${m.id}`} className="inline-flex items-center gap-1 text-[12px] font-medium text-slate-700 hover:text-slate-900">
