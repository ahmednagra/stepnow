...
// Testimonials list. Search, preview, exports, rating column, bilingual stacked quotes.
  ...
  return (
    <span className="inline-flex items-center gap-0.5" aria-label={`${value} of 5`}>
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          key={i}
          className={i < value ? "h-3 w-3 fill-[#A8865A] text-[#A8865A]" : "h-3 w-3 text-slate-200"}
          strokeWidth={1.5}
          aria-hidden="true"
        />
      ))}
                ...
                <AdminTableRow key={t.id} className={t.is_deleted ? "opacity-60" : ""}>
                  ...
                  <AdminTableCell>
                    <div className="flex max-w-md flex-col gap-1">
                      <p className="line-clamp-2 text-[12.5px] italic text-slate-700">
                        <span className="mr-1 inline-block text-[9px] font-semibold uppercase tracking-[0.14em] not-italic text-slate-400">
                          DE
                        </span>
                  ...
                  <AdminTableCell>
                    {t.is_deleted ? (
                      ...
                      <span className="inline-block bg-emerald-50 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wider text-emerald-700">
                        Active
                      </span>
                    ) : (
                      <span className="inline-block bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wider text-slate-500">
