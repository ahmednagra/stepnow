          ...
          <ScrollReveal as="ul" stagger className="grid gap-px overflow-hidden border border-line bg-line md:grid-cols-2">
            {services.map((s) => (
              <li key={s.id} className="bg-cream">
                <Link href={`/en/services/${s.slug}`} className="group relative flex h-full flex-col gap-4 p-8 transition-all duration-base ease-out-premium hover:shadow-ring-ink md:p-10">
