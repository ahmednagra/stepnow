...
return (
<div>
<div role="tablist" aria-label={pickT(t, "pricing.tabs.aria_label", "Service pricing")} className="flex gap-0 overflow-x-auto border-b border-line [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
...
return (
<button key={service.id} role="tab" type="button" aria-selected={isActive} onClick={() => handleTabClick(service.slug)} className={cn("inline-flex items-center gap-2 whitespace-nowrap border-b-2 px-5 py-3.5 text-[13px] font-medium transition-colors duration-base -mb-px", isActive ? "border-gold text-ink font-semibold" : "border-transparent text-mute hover:text-ink")}>
<Icon className={cn("h-3.5 w-3.5", isActive ? "text-gold-deep" : "text-mute-soft")} strokeWidth={1.5} aria-hidden="true" />
<span>{service.title}</span>
{count > 0 && (
<span className={cn("ml-1 inline-block min-w-[24px] rounded-full border px-1.5 py-0.5 text-center text-[10.5px] tabular-nums", isActive ? "border-gold/30 text-gold-deep" : "border-line text-mute-soft")}>{count}</span>
)}
</button>
);
})}
...
<p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-gold">{service.title}</p>
...
{service.short_description && (<p className="mt-2 text-[13px] text-cream/75">{service.short_description}</p>)}
</div>
</div>
<div className="pt-7">
...
<h4 className="text-[15px] font-semibold tracking-tight text-ink">{category.name}</h4>
<span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-mute-soft">
{category.items.length}
{locale === "de" ? " Strecken" : " routes"}
</span>
</div>
{category.description && (<p className="mb-3 text-[13px] text-mute">{category.description}</p>)}
<table className="w-full border-collapse text-left">
<thead className="hidden md:table-header-group">
<tr>
<th scope="col" className="py-2.5 pr-4 text-[10.5px] font-semibold uppercase tracking-[0.18em] text-mute-soft" style={{ width: "30%" }}>{locale === "de" ? "Von" : "From"}</th>
<th scope="col" className="py-2.5 pr-4 text-[10.5px] font-semibold uppercase tracking-[0.18em] text-mute-soft" style={{ width: "50%" }}>{locale === "de" ? "Nach" : "To"}</th>
<th scope="col" className="py-2.5 text-right text-[10.5px] font-semibold uppercase tracking-[0.18em] text-mute-soft" style={{ width: "20%" }}>{locale === "de" ? "Preis" : "Price"}</th>
</tr>
</thead>
<tbody>
{category.items.map((item) => (
<tr key={item.id} className="block border-b border-line-soft transition-colors hover:bg-paper md:table-row">
<td className="block py-3 pr-0 align-top text-[14.5px] font-medium text-ink md:table-cell md:pr-4 md:py-3.5">{item.from_location}</td>
...
<td className="block py-1 text-left align-top font-serif text-[20px] font-medium tabular-nums text-gold-deep md:table-cell md:py-3.5 md:text-right md:text-[22px]">{formatPrice(item.price_eur, locale)}</td>
</tr>
))}
</tbody>
</table>
...
{pickT(t, "pricing.footnote", locale === "de" ? "Preise gelten für Einzelfahrten mit bis zu 4 Fahrgästen. Größere Gruppen, Rückfahrten und Nachtzuschläge (22:00–06:00) auf Anfrage. Andere Strecken — Festpreis-Angebot innerhalb von 30 Minuten während der Telefonzeiten." : "Prices valid for one-way bookings with up to 4 passengers. Larger groups, return trips, and night surcharges (22:00–06:00) on request. Other routes — fixed-price quote within 30 minutes during phone hours.")}
</div>
</>
)}
</div>
