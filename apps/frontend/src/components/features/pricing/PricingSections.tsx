...
flughafentransfer: "https://images.unsplash.com/photo-1620227134464-f879b1b93807?w=1800&q=80&auto=format&fit=crop",
krankenhausfahrten: "https://images.unsplash.com/photo-1626058770278-b0abe39bedfd?w=1800&q=80&auto=format&fit=crop",
schuelerbefoerderung: "https://images.unsplash.com/photo-1471174617910-3e9c04f58ff5?w=1800&q=80&auto=format&fit=crop",
"shuttle-service": PRICING_HERO_FALLBACK_URL,
"airport-transfer": "https://images.unsplash.com/photo-1620227134464-f879b1b93807?w=1800&q=80&auto=format&fit=crop",
"hospital-transport": "https://images.unsplash.com/photo-1626058770278-b0abe39bedfd?w=1800&q=80&auto=format&fit=crop",
"school-transport": "https://images.unsplash.com/photo-1471174617910-3e9c04f58ff5?w=1800&q=80&auto=format&fit=crop",
};
...
export function PricingFeaturedHero({ t, locale, settings, featuredItem, backgroundImageUrl, bookingHref }: PricingFeaturedHeroProps) {
...
interface PricingTrustStripProps { t: TFunction; locale: Locale; }
...
const after = pickT(t, "pricing.trust.after", locale === "de" ? " keine versteckten Aufschläge, kein Überraschungspreis am Zielort." : " no hidden surcharges, no surprise total at the destination.");
...
{ key: "luggage", defaults: { de: { label: "Standardgepäck", desc: "— 1 Koffer + 1 Handgepäck pro Fahrgast." }, en: { label: "Standard luggage", desc: "— 1 case + 1 cabin bag per passenger." } } },
{ key: "waiting", defaults: { de: { label: "15 Minuten Wartezeit", desc: "— 60 Minuten am Flughafen mit Meet & Greet." }, en: { label: "15 minutes waiting time", desc: "— 60 minutes for airport meet & greet." } } },
{ key: "childseat", defaults: { de: { label: "Kindersitz oder Sitzerhöhung", desc: "— auf Anfrage, kostenfrei." }, en: { label: "Child seat or booster", desc: "— on request, free of charge." } } },
{ key: "flighttrack", defaults: { de: { label: "Flugverfolgung", desc: "— wir passen die Abholzeit bei Verspätungen an." }, en: { label: "Flight tracking", desc: "— we adjust pickup if your flight is delayed." } } },
];

interface PricingIncludedMomentProps { t: TFunction; locale: Locale; }
...
interface PricingExcludedStripProps { t: TFunction; locale: Locale; }
...
key: string;
defaults: { de: { label: string; stepnow: string; taxi: string }; en: { label: string; stepnow: string; taxi: string } };
}

const COMPARISON_ROWS: ComparisonRow[] = [
{ key: "row1", defaults: { de: { label: "Preis Esslingen → STR", stepnow: "Festpreis €70 — bei Buchung bestätigt", taxi: "Taxameter ca. €80–110, vom Verkehr abhängig" }, en: { label: "Price for Esslingen → STR", stepnow: "Fixed €70 — confirmed at booking", taxi: "Meter-based ≈ €80–110, depends on traffic" } } },
{ key: "row2", defaults: { de: { label: "Fahrer zugewiesen", stepnow: "Gleiche Nummer, gleicher Fahrer, jedes Mal", taxi: "Wen auch immer die Zentrale schickt" }, en: { label: "Driver assigned", stepnow: "Same number, same driver, every time", taxi: "Whoever dispatch sends" } } },
{ key: "row3", defaults: { de: { label: "Im Voraus gebucht", stepnow: "Online oder telefonisch vorbestellt", taxi: "Am Tag selbst herangerufen, Verfügbarkeit unklar" }, en: { label: "Booked in advance", stepnow: "Pre-booked online or by phone", taxi: "Hailed on the day, hope for availability" } } },
{ key: "row4", defaults: { de: { label: "Meet & Greet am Flughafen", stepnow: "Bei Flughafenstrecken inklusive", taxi: "Zusatzservice falls verfügbar" }, en: { label: "Meet & greet at airport", stepnow: "Included with airport routes", taxi: "Extra service if available" } } },
{ key: "row5", defaults: { de: { label: "Rechnung für Geschäftskunden", stepnow: "Detaillierte Rechnung nach Vereinbarung", taxi: "Handgeschriebene oder gedruckte Quittung" }, en: { label: "Receipt for business", stepnow: "Detailed invoice, by arrangement", taxi: "Handwritten or printed slip" } } },
];

interface PricingComparisonProps { t: TFunction; locale: Locale; }
...
interface PricingPaymentCancellationProps { t: TFunction; locale: Locale; agbHref: string; }
