    ...
    <div>
        ...
        <div className="relative mt-6 h-[260px] border border-[color:var(--color-border-soft)] bg-[var(--color-bg-page)]">
          ...
          {osmHref && (
            <a
              ...
              className="absolute bottom-3 left-3 z-[400] border border-[color:var(--color-border-soft)] bg-[color:rgba(255,253,248,0.94)] px-3 py-1.5 text-[10.5px] font-semibold uppercase tracking-[0.14em] text-[var(--color-text-primary)] transition-colors duration-base hover:text-[var(--color-accent-primary)]"
            >
              {pickT(
                t,
                "about.area.open_external",
                locale === "de" ? "In Karten öffnen" : "Open in maps",
              )}{" "}
              ↗
            </a>
          )}
      ...
      <p className="mt-4 text-[13px] leading-relaxed text-[var(--color-text-secondary)]">
        {pickT(
          ...
          "about.area.body",
          locale === "de"
            ? "Wir fahren in Plochingen, Esslingen, Deizisau und im mittleren Neckartal."
            : "We operate in Plochingen, Esslingen, Deizisau and the central Neckar valley.",
        )}
      </p>
    </div>
  );
...
export const ServiceAreaMap = memo(
  ServiceAreaMapImpl,
  (p, n) =>
