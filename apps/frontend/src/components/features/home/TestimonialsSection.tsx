...
// Rotating editorial quote. Pauses on hover, off-screen (IntersectionObserver), tab-hidden (Page Visibility), and prefers-reduced-motion.
...
>
<span aria-hidden="true" className="block font-serif text-[5rem] leading-none text-gold/30 md:text-[6rem]">“</span>
{current.rating !== null && current.rating > 0 && <RatingStars value={current.rating} />}
<blockquote key={current.id} className="mt-3 font-serif text-2xl leading-relaxed text-ink animate-fade-in md:text-3xl md:leading-[1.35]">
{current.quote}
</blockquote>
...
onClick={() => setIdx(i)}
className={cn("h-px w-10 transition-colors duration-base", i === idx ? "bg-ink" : "bg-line hover:bg-mute")}
/>
))}
</div>
)}
...
<Star key={i} aria-hidden="true" strokeWidth={1.25} className={cn("h-4 w-4", i < value ? "fill-gold text-gold" : "text-line-strong")} />
))}
</div>
);
}
