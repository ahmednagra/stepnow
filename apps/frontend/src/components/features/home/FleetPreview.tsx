...
// Magazine-grid fleet section. Uses next/image (AVIF/WebP + responsive srcset); hover scale gated to md+ to avoid mobile compositing overhead.
...
<ul className={cn("grid gap-6", count === 1 ? "md:mx-auto md:max-w-2xl md:grid-cols-1" : count === 2 ? "md:grid-cols-2" : "md:grid-cols-3")}>
...
<>
<div aria-hidden="true" className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(168,134,90,0.22),transparent_65%)]" />
<p className="absolute bottom-5 left-5 right-5 font-serif text-[26px] leading-[1.1] tracking-tight text-cream/90 md:text-[28px]">{vehicle.name}</p>
</>
)}
{categoryLabel && (
...
<h3 className="font-serif text-[22px] font-medium leading-tight tracking-tight text-ink">{vehicle.name}</h3>
{vehicle.features.length > 0 && (
<ul className="flex flex-wrap gap-1.5">
{vehicle.features.slice(0, 4).map((feature) => (
<li key={feature} className="border border-line-soft bg-paper px-2.5 py-1 text-[11px] tracking-tight text-mute">{feature}</li>
))}
</ul>
)}
