...
import {
  LayoutDashboard, Briefcase, Car, MessageSquareQuote, HelpCircle, Tags,
  Settings, CalendarCheck, Mail, FileText, Languages, AtSign, History, Users,
} from "lucide-react";
...
const NAV: NavItem[] = [
  ...
  { href: "/admin/bookings", label: "Bookings", icon: CalendarCheck, group: "Operations", badgeKey: "bookings" },
...
function AdminSidebarBase({ counts }: AdminSidebarProps) {
  ...
  return (
    <aside className="hidden h-full w-60 shrink-0 border-r border-slate-200 bg-white lg:flex lg:flex-col">
      ...
      <nav className="flex flex-1 flex-col gap-0.5 overflow-y-auto px-3 py-4 scrollbar-dark">
        {NAV.map((item) => {
            ...
            <div key={item.href}>
              ...
              {item.comingSoon ? (
                <div className="flex items-center justify-between gap-2 px-3 py-1.5 text-[13px] text-slate-400">
                  ...
                  <span className="text-[9px] font-semibold uppercase tracking-[0.16em] text-slate-300">Soon</span>
                </div>
              ) : (
                <Link
                  href={item.href}
                ...
                >
                  {isActive && <span aria-hidden="true" className="absolute left-0 top-1 bottom-1 w-[2px] bg-[#A8865A]" />}
                  <span className="flex items-center gap-2.5">
                    <Icon
